...
class UserRepository:
    ...
    def list(self, skip: int = 0, limit: int = 100, sort: str | None = None) -> list[User]:
        stmt = select(User).offset(skip).limit(limit).order_by(getattr(User, sort))
    ...
    def get_by_email(self, email: str) -> User | None:
        result = self.db.scalar(select(User).where(User.email == email))
        return result

    def create(self, payload: UserCreateSchema) -> User:
    ...
    def update(self, user_id: int, payload: UserUpdateSchema) -> User | None:
